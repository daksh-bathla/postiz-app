...
async function fetchRealPosts(queries: string[]): Promise<string> {
...
export class GrowthController {
  ...
  async generateOpportunities(@Req() req: Request, @Body() body: any) {
    ...
    if (!product?.name) {
      return { error: 'Product info required.' };
    }
...
Extract 3-5 real opportunities. Output JSON:
{
  "extractedPosts": [
    ...
    try {
      const raw = await makeAiRequest(discoveryPrompt, modelConfig);
      discoveryResult = JSON.parse(stripJsonFences(raw));
    } catch (e: any) {
      return { error: `Discovery failed: ${e.message}`, opportunities: [] };
    }
...
Output JSON:
{
  ...
  "expectedOutcome": "..."
          ...
          const raw = await makeAiRequest(writerPrompt, modelConfig);
  ...
  async autoFill(@Body() body: any) {
    ...
    try {
      ...
      if (
        ...
        return { error: 'Private URLs not allowed.' };
      }
    } catch {
...
Website: ${text}`;
      ...
      const productContext = JSON.parse(stripJsonFences(raw));
